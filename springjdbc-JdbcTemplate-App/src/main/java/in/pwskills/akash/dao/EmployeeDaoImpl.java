...
public class EmployeeDaoImpl implements IEmployeeDao {
	...
	private static final String GET_EMPLOYEE_DETAILS_BY_ENO = "select * from employee where empNo = ? ";

	private static final String GET_EMP_DETAILS_BY_DESG = "select * from employee where job in (?,?) order by job";
	...
	private static final String DELETE_EMP_BY_SALARY = "delete from employee where salary>=? and salary<=? ";
	...
	public Map<String, Object> getEmployeeByEno(Integer eid) {
		return template.queryForMap(GET_EMPLOYEE_DETAILS_BY_ENO,eid);
	}

	@Override
	public List<Map<String, Object>> getEmployeeByDesg(String desg1, String desg2) {		
		return template.queryForList(GET_EMP_DETAILS_BY_DESG, desg1,desg2);
	}
